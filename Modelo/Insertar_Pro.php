...
class Flyweight
{
    ...
    public function operation($uniqueState): void
    {
        ...
        // echo "<p> Flyweight: Visualizacion del estado compartido ($s) y unico ($u) .\n";
    }
}
...
class FlyweightFactory
{
    ...
    public function getFlyweight(array $sharedState): Flyweight
    {
        ...
        if (!isset($this->flyweights[$key])) {
            // echo "<p> FlyweightFactory: No se puede encontrar FlyWeight, creando uno nuevo.\n";
            $this->flyweights[$key] = new Flyweight($sharedState);
        } else {
            // echo "<p> FlyweightFactory: Re-haciendo FlyWeight existente.\n";
        }
    ...
    public function listFlyweights(): void
    {
        ...
        // echo "<p>\nFlyweightFactory: Se tiene $count FlyWeights:\n";
...
function filtrado($dato)
{
    ...
    $dato = stripslashes($dato); // Elimina los backslash
...
if(isset($_POST['Agregar']))
{
	...
	{
	    if (!((strpos($tipo, "gif") || strpos($tipo, "jpeg") || strpos($tipo, "jpg") || strpos($tipo, "png")) && ($tamanio < 2000000)))
	    {
	       ?>
    ...
    (`product_id`, `product_cat`, `product_brand`, `product_title`, `product_price`, `product_desc`, `product_image`, `product_keywords`, `Stock`, `IVA`, `Descuento`) 
    VALUES (NULL, '$cat', '$marca', '$nombre', '$precio', '$desc', '$imagen', '$nombre', '$stock', '$iva', '$descuento');"; 
    ...
    if(mysqli_query($con, $sentencia))
	{
        ...
        Swal
        .fire({
        title: 'Nuevo producto agregado a la Tienda',
        ...
        if (resultado.value)
        {
            ...
            if (res.value)
            {
                Swal.fire(console.log(window.location = "../Vista/Agregar_Pro.php"));
            }
            });
        }
        });
    ...
    <a href="../Vista/Agregar_Pro.php?ID=<?php echo $id; ?>"><button class="btn btn-success" id="Volver" width="700px" height="100px"><font size=3 face="Verdana"><b> Volver </b></font></button></a>
...
([
    ["<p> * NULL", "$cat", "$marca", "$nombre", "$precio", "$desc", "$imagen", "$desc", "$stock", "$iva", "$descuento", "<p>"],
   // ["<p> * Acer", "Intel Core i7", "Negro <p>"],
   //["<p> * Acer", "Intel Core i5", "Blanco <p>"],
   //["<p> * Apple", "iPhone", "Negro <p>"],
   //["<p> * Apple", "iPad", "Gray <p>"],
    // ...
]);
...
function Agregar (FlyweightFactory $id, $cat, $marca, $precio, $desc, $stock, $iva, $descuento) 
{
    // echo "<p>\nA単adiendo producto a la base de datos.\n";
    $flyweight = $id->getFlyweight([$precio, $desc, $stock, $iva, $descuento]);

    // The client code either stores or calculates extrinsic state and passes it
    // to the flyweight's methods.
    $flyweight->operation([$cat, $marca]);
}
